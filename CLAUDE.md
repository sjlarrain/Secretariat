...
- **WhatsApp always gets 200**: Webhook route must always return HTTP 200 to Kapso, even on errors, to prevent retries. Errors are sent as WhatsApp replies. **No exceptions** — the v1 proxy briefly had one (a 502 when the forward failed, to buy Kapso's retries) and it lost messages; see the next bullet.
- **Kapso's contract, measured against the vendor docs, not against what the code assumed**: the endpoint must return **200 within 10 seconds**; anything else — including a timeout — is a failed delivery, retried at **+10s, +40s, +90s** (3 retries, ~2.5 min total). A webhook is **auto-paused** (`active: false`, pending messages dropped with "Webhook inactive; delivery skipped") at ≥20 deliveries **and** ≥10 failures **and** ≥85% failure rate inside a 15-minute window. Any design that answers slowly or non-200 on purpose is spending down that budget.
...
- **Inbound routing (v2): v2 owns the single Kapso webhook and forwards Santiago's number to v1.** Kapso allows one raw-webhook subscription per phone number, so one service is the front door for the shared number, and it is v2 — v1's code is then untouched. `v1ProxyMiddleware` (`platform/v1-proxy.ts`) is mounted **before** `resolveSenderMiddleware`, because once Santiago's number is also registered in v2 for testing, `resolveSender()` returns `kind: 'user'` and a check placed after it would never fire. It forwards the raw `req.body` — never reconstructed fields — so v1's own `normalizeWebhook()` and message-id dedup keep working. Enabled by `V1_WEBHOOK_URL`; unsetting it is the cutover.
  - **The forward is acked first and delivered afterwards, and this is not negotiable.** v1 is on Render's free tier: it spins down after 15 minutes idle, and Render's router then **holds** the next request open for 30–60s while the instance boots — it does not reject it and it does not retry it. Nothing in the chain retries the v2→v1 hop, so the *only* thing deciding whether a cold-start forward survives is how long v2 is willing to wait. The original code aborted at 10s and answered 502: every first message after an idle period woke v1 and was then thrown away, while Kapso's retry landed inside the still-held dedup claim and got a `200 proxied-v1-duplicate`, ending the retry chain. Kapso's dashboard showed those as delivered. So: claim dedup → write the payload to `sys:v1-pending` → **ack 200** → forward in the background. The background forward must survive **both** shapes a cold start takes, and covering only one is why this needed two attempts: Render's router either **holds** the request for the 30-60s spin-up (survived by `FORWARD_TIMEOUT_MS` = 75s; Node does not cap this — undici's default headers timeout is 300s) **or answers 502/503 immediately** while the instance boots, where no timeout is involved at all and only retrying later helps (`RETRY_DELAYS_MS` = 10s/40s/90s, deliberately the same ladder Kapso uses, capped by `FORWARD_DEADLINE_MS`). Never shorten the budget below a cold start, never collapse the retry ladder back to a couple of seconds, and never move the ack after the forward.
  - **v1 is woken on demand, never on a schedule.** Before forwarding, `waitForV1Awake()` polls v1's `/health` (derived from `V1_WEBHOOK_URL`) every 5s up to 90s, and the payload is POSTed only once v1 answers. Two reasons, and both matter: repeatedly POSTing the real payload at a spun-down free instance is what Render's edge throttles — it returned **429 to every forward while `GET /health` from an ordinary client returned 200** — and a schedule-based keep-alive would burn the workspace's 750 free instance hours/month, which two always-on free services cannot both fit inside. Knocking costs one cheap GET when v1 is already up and nothing at all when nobody is writing. Do not replace this with a cron keep-alive.
  - **`sys:v1-pending` is the whole safety net.** Acking up front forfeits Kapso's retries, so a message that the background forward fails to deliver is recoverable only from that hash. `redriveV1Forwards()` re-drives it once per hourly sweep and drops entries older than 24h. Do not remove the redrive from `runSweep()` while `V1_WEBHOOK_URL` is set. Do not add a second Kapso subscription for v2 — a fan-out would need v1's `whitelistMiddleware` edited to stop replying to v2's users. See `docs/v2-plan.md` §C.8.

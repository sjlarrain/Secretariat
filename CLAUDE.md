...
- **Inbound routing (v2): a Kapso Function is the front door.** Kapso allows one webhook per phone number, so one thing routes for the shared number — a Kapso Function on Cloudflare that sends Santiago's number to v1's webhook and everyone else to v2's. v2 does **not** talk to v1: the earlier in-process proxy (`platform/v1-proxy.ts`, `sys:v1-pending`, `redriveV1Forwards`, `V1_WEBHOOK_URL`) was deleted because the Render→Render hop it needed is refused with 429 on every attempt. Do not reintroduce it, a second Kapso subscription, or a cold-start retry ladder — Kapso's own retries (+10s/+40s/+90s) plus v1's 5-minute dedup already cover v1 spinning up. Design, function source, secrets, and cutover: **`docs/v1-routing-fix.md`**.

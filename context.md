...
- QStash free tier (verified 2026-07-16): **1,000 messages/day, 10 active schedules, 7-day max delay**. Secretariat registers at most 6 schedules, so the count fits the free tier. The **7-day delay cap is the real constraint** — it is why `/reminder` defers anything further out and why the reminder promoter cron exists. With the promoter disabled, deferred reminders never fire. The health check reports schedules that are enabled but have no live schedule ID

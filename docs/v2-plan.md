...
- **`sys:v1-pending` replaces the upstream retry.** Acking forfeits Kapso's retries, so an undelivered payload lives in that hash until `redriveV1Forwards()` — one step of the hourly sweep — gets it into v1, or gives up at 24h. Regression coverage is in `__tests__/v1-proxy.test.ts` under "cold-start delivery": those tests fail if the forward budget is ever cut back to Kapso's 10s window.
